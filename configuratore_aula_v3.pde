...
int btnY_pos = 50;
int btnY_dist = 30;
int btn_num = 30; // parametri aula misurabili complessivamente
int btn_num_ON = 17; // misure visibili modificabili a video dall'utente
...
void setup() { // setup valori iniziali  
    ...
    misure[15] = 0; // offset posizione porta
    misure[16] = 90; // larghezza porta
    ...
    incrementi[15] = 5; // incremento posizione porta
    incrementi[16] = 1; // incremento larghezza porta
    ...
    min_max_valori[15][0] = -95; // pos porta
    min_max_valori[15][1] = misure[1]-190; // pos porta 

    min_max_valori[16][0] = 60; // pos porta
    min_max_valori[16][1] = 200; // pos porta 
    ...
    btn_label[15] = "pos porta";
    btn_label[16] = "largh porta";
    btn_label[btn_num_ON] = "Aula X";
    btn_label[btn_num_ON+1] = "Aula Y";
    
    c = new GUIController(this);
    t1 = new IFTextField("Text Field", btnX_pos, btnY_pos + btnY_dist * btn_num_ON, 50, str(misure[0]));
    t2 = new IFTextField("Text Field", btnX_pos, btnY_pos + btnY_dist * (btn_num_ON + 1), 50, str(misure[1]));
    l1 = new IFLabel(btn_label[(btn_num_ON + 0)], btnX_pos + 60, btnY_pos + (btn_num_ON + 0) * btnY_dist);
    l2 = new IFLabel(btn_label[(btn_num_ON + 1)], btnX_pos + 60, btnY_pos + (btn_num_ON + 1) * btnY_dist);
    txtNomeAula = new IFTextField("Text Field", btnX_pos, btnY_pos + btnY_dist * (btn_num_ON + 2), 170, "Nome aula");
    b = new IFButton("Calcola", btnX_pos, btnY_pos + btnY_dist * (btn_num_ON + 3), 50);
    b1 = new IFButton("Fai PDF", btnX_pos, btnY_pos + btnY_dist * (btn_num_ON + 4), 50);
    b2 = new IFButton("Lavoro basato sul progetto del prof. Omar Lazzari", 150, 10, 300);
    b3 = new IFButton("Licenza CC-BY-SA", 490, 10, 120);
    b4 = new IFButton("Autore: D.S. Davide Cammarota", 650, 10, 300);
    b5 = new IFButton("Fine lavoro", btnX_pos, btnY_pos + btnY_dist * (btn_num_ON + 5), 50);
...
void disegna_banchi(int x0, int y0, int aulaX, int aulaY, int bancoX, int bancoY, int fondo_aula, int spazio_insY, int cattedraX, int cattedraY, int dist_professore, int pos_cattedra, int pass_agg) {
    ...
    for (int i = 0; i < numerofile + 1; i++) { // disegno banchi - attenzione numerofile è pari al numero di file -1! - idem per numerorighe
        ...
        if (pass_agg > 0 && i == (int) numerofile / 2) {  // aumento il passaggio aggiuntivo solo in un corridoio centrale
            xs = xs + interasseX + pass_agg;
        } else xs = xs + interasseX;
    }

    xs = x0; // banchi a fianco della cattedra
    for (int i = 0; i < misure[14]; i++) {    // misure[14] banchi aggiuntivi
    ...
    rect(x0 + aulaX, y0 + 100 - 5 + misure[15], misure[16], 5); // disegna porta ingresso
    arc(x0 + aulaX, y0 + 100 + misure[15], misure[16]*2, misure[16]*2, 0, 3.14 / 2);
...
void calcolo_interassi(int aulaX, int aulaY, int bancoX, int bancoY, int interX, int interY, int fondo_aula, int bordo_aula, int sp_insegnante, int pass_agg){
   ...
   if (bordo_aula_def < max(bancoX/2, bordo_aula)){
     numerofile--;
     bordo_aula_def = ((aulaX-pass_agg-numerofile*interX)/2);
   }
...
void actionPerformed(GUIEvent e) {
    ...
    if (e.getSource() == b2) {
        link("https://github.com/omartek/calcolaclassi");
    }
    if (e.getSource() == b3) {
        link("https://creativecommons.org/licenses/by-sa/2.0/");
    }
    if (e.getSource() == b4) {
        link("https://www.iissmedi.edu.it/attachments/article/821/CV_Davide_Cammarota.pdf");
    }
    if (e.getSource() == b5) {
        exit();
    }
